...
func registerCaptureHotkey(app *thawtsapp.App) {
	go func() {
		...
		// Ctrl+Alt+Space avoids the Ctrl+Shift language-switching intercept
		// that is common on Windows and prevents WM_HOTKEY delivery.
		ret, _, err := registerHotKeyProc.Call(0, hotkeyID, modCtrl|modAlt|modNoRepeat, vkSpace)
		if ret == 0 {
			log.Printf("WARNING: global hotkey registration failed: %v", err)
			log.Printf("         Ctrl+Alt+Space will not work. Use the system tray icon to open the capture window.")
			return
		}
		log.Printf("global hotkey Ctrl+Alt+Space registered successfully")
		...
		for {
			...
			if m.Message == wmHotkey && m.WParam == hotkeyID {
				log.Printf("Ctrl+Alt+Space keydown received")
